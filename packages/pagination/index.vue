<template>
  <div class="pagination">
    <ul :class="['pagination-inner', { 'has-background': background }]">
      <li :class="['pagination-item', 'pre', { 'is-disabled': !canPre }]"
          @click.stop="handleClick({ type: 'pre' })">
        <hIcon name="keyboard_arrow_left"
                  :size="12" />
      </li>
      <!-- backward Paginations -->
      <li v-for="item in forwardPaginations"
          :key="item"
          :class="['pagination-item', { 'is-active': item === current }]"
          @click.stop="handleClick({ type: 'select', data: item })">{{ item }}</li>

      <li v-if="isForwardBrief"
          class="pagination-item icon-more">
        <hIcon name="more_horiz"
                  :size="12" />
      </li>
      <!-- end -->
      <!-- middle paginations -->
      <template v-if="isForwardBrief && isBackwardBrief">
        <li v-for="item in middlePaginations"
            :key="item"
            :class="['pagination-item', { 'is-active': item === current }]"
            @click.stop="handleClick({ type: 'select', data: item })">{{ item }}</li>
      </template>
      <!-- end -->
      <!-- backward paginations -->
      <li v-if="isBackwardBrief"
          class="pagination-item icon-more">
        <hIcon name="more_horiz"
                  :size="12" />
      </li>

      <li v-for="item in backwardPaginations"
          :key="item"
          :class="['pagination-item', { 'is-active': item === current }]"
          @click.stop="handleClick({ type: 'select', data: item })">{{ item }}</li>
      <!-- end -->
      <li :class="['pagination-item', 'next', { 'is-disabled': !canNext }]"
          @click.stop="handleClick({ type: 'next' })">
        <hIcon name="keyboard_arrow_right"
                  :size="12" />
      </li>
    </ul>
  </div>
...
export default {
  ...
  computed: {
    ...
    backwardPaginations() {
      const { pageCount, isBackwardBrief, paginalNumber } = this;
